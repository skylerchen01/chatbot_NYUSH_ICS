...
def startChatting():
    ...
    def game():
        print('game')
        if client.sm.get_state() == S_CHATTING:
            client.sm.set_state(S_GAMING)
        plane = Game.Plane.gameinit(client.socket)
        plane.run()
    ...
    gameBtn.grid(row = 0,column = 2,sticky = W,padx=5,pady=10)
    musicBtn.grid(row=0,column=3,padx=5, pady=10)
    stopBtn.grid(row=0,column=4,padx=5, pady=10)
    poemBtn.grid(row = 0,column = 5,padx=5,pady=10)
